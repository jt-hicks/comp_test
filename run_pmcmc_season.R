...
run_pmcmc <- function(data_raw,
  ...
  pretime <- as.integer(difftime(start_obs,start_seas,units="days"))
  data_raw_time <- data_raw %>%
    mutate(date = as.Date(as.yearmon(month), frac = 0.5))%>%
    mutate(t = as.integer(difftime(date,start_seas,units="days")))

  preamble <- data.frame(t=as.integer(difftime(seq.Date(start_seas,start_obs,by='month'),start_seas,units="days")))
  data_shift <- plyr::rbind.fill(tail(preamble,-1),data_raw_time)
  data <- mcstate::particle_filter_data(data_shift, time = "t", rate = NULL, initial_time = 0)
  
  compare <- function(state, observed, pars = NULL) {
    if (is.na(observed$tested)) {
      ll <- numeric(length(state[1,]))
    } else {
  ...
  index <- function(info) {
    list(run = c(prev = info$index$prev),
         state = c(prev = info$index$prev,
                   ...
                   inc = info$index$inc,
                   check_log_EIR = info$index$check_log_EIR,
                   log_eir = info$index$log_eir))
  }
  
  stochastic_schedule <- as.integer(difftime(seq.Date(start_seas,max(as.Date(data_raw_time$date+30)),by='month'),start_seas,units="days"))[-1]
  ...
  model <- odin.dust::odin_dust("odinmodelmatchedstoch1.R")
