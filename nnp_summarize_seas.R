...
nnp_pg_list <- list(Banfora = data_raw_bf_pg_banfora,Gaoua = data_raw_bf_pg_gaoua,Orodara = data_raw_bf_pg_orodara,
                 ...
                 Asa = data_raw_ng_pg_asa[,1:4],Ejigbo = data_raw_ng_pg_ejigbo,`Ife North` = data_raw_ng_pg_ifenorth, Moro = data_raw_ng_pg_moro)
  ...
  for(i in start:(start+number-1)){
    data_cis <- addCIs(data_list[[i]],data_list[[i]]$positive,data_list[[i]]$tested)%>%
      mutate(district=districts[[i-start+1]],
             month = as.Date(as.yearmon(month), frac = 0.5))
    df_data <- rbind(df_data,data_cis)
    data_length <- nrow(data_cis)

    prev_history <- data.frame(t(results[[i]]$history['prev', 51:1000, ]))
    prev_history_obs <- prev_history[(nrow(prev_history)-data_length+1):nrow(prev_history),]
    long_prev_sum <- prev_history_obs%>% mutate(t=c(1:nrow(prev_history_obs)))%>%
      melt(id='t')%>%
      rename(time=t)%>%
      group_by(time)%>%
...
BF_obsprev_pgseas <- create_obsprev_plots(results = nnp_pgseas_result_list,data_list=nnp_pg_list,country='BF')
MZ_obsprev_pgseas <- create_obsprev_plots(results = nnp_pgseas_result_list,data_list=nnp_pg_list,country='BF')
NG_obsprev_pgseas <- create_obsprev_plots(results = nnp_pgseas_result_list,data_list=nnp_pg_list,country='NG')
...
create_estprev_plots <- function(results,data_list=nnp_list,country=c('BF','MZ','NG')){
  ...
  for(i in start:(start+number-1)){
    ...
    long_prev_sum <- prev_history_obs%>%
      mutate(t=c(1:nrow(prev_history_obs)))%>%
      melt(id='t')%>%
      rename(time=t)%>%
      group_by(time)%>%
      summarise(median=median(value),
    ...
    prev_sample <- prev_history_obs[, sample(ncol(prev_history_obs), 100)] %>%
      mutate(t=c(1:nrow(prev_history_obs)))%>%
      melt(id='t')%>%
      rename(time=t)%>%
      mutate(district = districts[[i-start+1]],
...
BF_estprev_pgseas <- create_estprev_plots(results = nnp_pgseas_result_list,data_list=nnp_pg_list,country='BF')
MZ_estprev_pgseas <- create_estprev_plots(results = nnp_pgseas_result_list,data_list=nnp_pg_list,country='MZ')
NG_estprev_pgseas <- create_estprev_plots(results = nnp_pgseas_result_list,data_list=nnp_pg_list,country='NG')
BF_estprev_pgseas / plot_spacer()
MZ_estprev_pgseas / plot_spacer()
NG_estprev_pgseas / plot_spacer()
windows(6,7)
